...
router.get('/health', async (req, res) => {
  ...
  try {
    ...
    const pyHealth = await new Promise((resolve, reject) => {
      lib.get(`${PYTHON_URL.origin}/api/v1/health`, (r) => {
        const chunks = [];
        r.on('data', c => chunks.push(c));
        r.on('end', () => {
          try { resolve(JSON.parse(Buffer.concat(chunks).toString())); }
          catch { resolve({ reachable: true, raw: true }); }
        });
      }).on('error', reject);
    });
    pythonStatus = { reachable: true, ...pyHealth.data };
  } catch {
    pythonStatus = { reachable: false, reason: 'Python microservice unreachable' };
  }
    ...
    try {
      if (!process.env.PYTHON_SERVICE_URL) {
        return res.status(503).json({
          success: false,
          error  : 'Python microservice is not configured (PYTHON_SERVICE_URL missing).',
        });
      }

      // Re-assemble the multipart body to forward to Python.
      // We use the raw buffer already held in memory by multer.
      ...
      const calibrationStr = typeof req.body.calibrationData === 'string'
      ...
      const { status, body } = await callPython('/api/v1/measure', bodyBuffer, contentType);
...
router.get('/measure/:jobId', async (req, res) => {
  try {
    ...
    const pyRes   = await new Promise((resolve, reject) => {
      lib.get(
        `${PYTHON_URL.origin}/api/v1/measure/${req.params.jobId}`,
        (r) => {
          const chunks = [];
          r.on('data', c => chunks.push(c));
    ...
    res.status(pyRes.status).set('Content-Type', 'application/json').send(pyRes.body);
  } catch {
    res.status(502).json({ success: false, error: 'Measurement service unreachable.' });
  }
});
